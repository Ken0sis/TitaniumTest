...
//Rare exceptions to indicate a bug in the _todolist code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_todolist = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_todolist]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.todolist.module"
